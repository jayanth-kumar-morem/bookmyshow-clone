...
function Datebox({selected, currentDate}:{selected: boolean, currentDate: Date}) {
    ...
    return (
        <a className={`date-box flex flex-col px-4 py-1 items-center ${selected ? "bg-red-500 text-white  border rounded-lg" : ""}`}>
            <span className={`uppercase m-0 ${!selected ? "text-slate-500" : "font-semibold"}`}
            style={{fontSize: "0.6rem"}}
            >{dayOfWeek}</span>
            <span className="m-0">{dayOfMonth}</span>
            <span className={`uppercase m-0 ${!selected ? "text-slate-500" : "font-semibold"}`}
            style={{fontSize: "0.6rem"}}
            >{monthAbbreviation}</span>
        </a>
    )
}
        ...
        <div className="ticket-options border-t-2">
            <div className="wrapper px-20 flex flex-row place-content-between items-center py-2">
                ...
                <div className="right flex flex-row">
                    <div className="lang border-b-red-500 px-4 mr-4 mt-5"
